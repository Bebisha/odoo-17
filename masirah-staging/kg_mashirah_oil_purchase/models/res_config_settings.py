...
class PurchaseOrderConfiguring(models.TransientModel):
    ...
    def set_values(self):
        ...
        self.env['ir.config_parameter'].sudo().set_param('kg_mashirah_oil_purchase.purchase_manager_ids',
                                                         self.purchase_manager_ids.ids)
        self.env['ir.config_parameter'].sudo().set_param('kg_mashirah_oil_purchase.general_manager_ids',
                                                         self.general_manager_ids.ids)
        self.env['ir.config_parameter'].sudo().set_param('kg_mashirah_oil_purchase.finance_manager_ids',
                                                         self.finance_manager_ids.ids)
        self.env['ir.config_parameter'].sudo().set_param('kg_mashirah_oil_purchase.line_manager_ids',
                                                         self.line_manager_ids.ids)
        self.env['ir.config_parameter'].sudo().set_param('kg_mashirah_oil_purchase.department_manager_ids',
                                                         self.department_manager_ids.ids)
        self.env['ir.config_parameter'].sudo().set_param('kg_mashirah_oil_purchase.hr_manager_ids',
    ...
    def get_values(self):
        ...
        purchase_manager = with_user.get_param('kg_mashirah_oil_purchase.purchase_manager_ids')
        general_manager = with_user.get_param('kg_mashirah_oil_purchase.general_manager_ids')
        finance_manager = with_user.get_param('kg_mashirah_oil_purchase.finance_manager_ids')
        line_manager = with_user.get_param('kg_mashirah_oil_purchase.line_manager_ids')
        department_manager = with_user.get_param('kg_mashirah_oil_purchase.department_manager_ids')
        hr_manager = with_user.get_param('kg_mashirah_oil_purchase.hr_manager_ids')
