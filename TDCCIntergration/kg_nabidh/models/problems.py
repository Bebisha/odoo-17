...
class KgProblems(models.Model):
    ...
    action_date_time = fields.Datetime(string="Action Date/Time", default=lambda self: fields.Datetime.now())
    problem_established_date = fields.Datetime(string="Problem Established Date/Time", default=lambda self: fields.Datetime.now())
    Problem_life_cycle_date = fields.Datetime(string="Problem Life Cycle Status Date/Time", default=lambda self: fields.Datetime.now())
    Problem_onset_date = fields.Datetime(string="Problem Date of Onset", default=lambda self: fields.Datetime.now())
...
class KgNotesComments(models.Model):
    ...
    def default_get(self, fields_list):
        """When 'Add a line' is clicked, fill default notes."""
        res = super(KgNotesComments, self).default_get(fields_list)
        res['note'] = 'Patient in good health condition.'
        res['source_comment'] = 'Clinical Note'
        return res

    @api.model
    def create(self, vals):
        if vals.get('set_id_nte', _('New')) == _('New'):
            vals['set_id_nte'] = self.env['ir.sequence'].next_by_code('set_id_nte') or _('New')
