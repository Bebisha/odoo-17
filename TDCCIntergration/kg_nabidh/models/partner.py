...
MAX_PDF_MB = 8  # adjust to your server limit (JSON+overhead!)
EMBED_PDF = True  # False => send only a URL/metadata
...
class Partner(models.Model):
    ...
    is_vip = fields.Boolean(string="VIP Patient", default=False)
    ...
    def _bytes_len_from_b64(self, b64s):
        """Return original binary size (bytes) from a base64 string."""
        # base64 size ≈ 4 * ceil(n/3); invert to estimate n
        # remove padding/newlines first
        s = b64s.strip().replace('\n', '')
        padding = s.count('=')
        return (len(s) * 3) // 4 - padding

    def _sha256_of_b64(self, b64s):
        ...
        for app in appointment:
            for observation in app.observations_ids:
                ...
                if observation.obs_atts:
                    if isinstance(observation.obs_atts, bytes):
                        pdf_b64 = observation.obs_atts.decode('utf-8')
                    else:
                        pdf_b64 = observation.obs_atts

                pdf_size_bytes = self._bytes_len_from_b64(pdf_b64) if pdf_b64 else 0
                pdf_size_mb = pdf_size_bytes / (1024 * 1024)
                pdf_sha256 = self._sha256_of_b64(pdf_b64) if pdf_b64 else None
                pdf_payload = ''
                pdf_included = False
                pdf_note = None
                ...
                if pdf_b64:
                    if EMBED_PDF:
                        if pdf_size_mb <= MAX_PDF_MB:
                            ...
                            pdf_note = f"PDF omitted: {pdf_size_mb:.2f} MB exceeds {MAX_PDF_MB} MB limit"
                    else:
                        pdf_note = "PDF sent via URL (not embedded)"

                obx = {
                    "valueType": observation.value_type.code,  # e.g., "ED" for encapsulated data
                    "observationId": f"{observation.observation_identifier.code}^{observation.observation_identifier.name}^NAB035",
                    "value": observation.observation_value,
                    "units": observation.units,
                    "abnormalFlags": observation.abnormal_flag_id.code,
                    "resultStatus": observation.observation_result_status.code,
                    "timestamp": app.date.strftime('%Y%m%d%H%M%S') if app.date else None,
                    "pdfData": pdf_payload if pdf_included else None,
                    "pdfMeta": {
                        "mimeType": "application/pdf" if pdf_b64 else None,
                        "sizeBytes": pdf_size_bytes,
                        "sizeMB": round(pdf_size_mb, 3),
                        "sha256": pdf_sha256,
                        "embedded": pdf_included,
                        "note": pdf_note,
                    }
                }
                OBX.append(obx)
