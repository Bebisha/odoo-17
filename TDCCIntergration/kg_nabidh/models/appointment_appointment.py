...
class AppointmentsAppointment(models.Model):
    ...
    def action_checkin(self):
        ...
        self.message_post(body=f"A04(Checkin) request sent to NABIDH. \n Nabidh Status: {nabidh_status}")
        self.is_checkin = True
        self.is_checkout = False

    def discharge_event(self):
        nabidh_status = self.client_id.discharge_event('A03',appointment=self)
        self.message_post(body=f"A03(Discharge)  sent to NABIDH. \n Nabidh Status: {nabidh_status}")
    ...
    def problem_record_pc1(self):
        ...
        self.message_post(body=f"PC1(Problem Record message) sent to NABIDH.\n Nabidh Status: {nabidh_status}")
    ...
    def unsolicited_transmission_observation(self):
        ...
        self.message_post(body=f"R01(Observation Message) sent to NABIDH. \n Nabidh Status: {nabidh_status}")

    def action_vo4(self):
        self.ensure_one()
        nabidh_status = self.client_id.action_vo4('V04',appointment=self)
        self.message_post(body=f"V04(Vaccination message) sent to NABIDH. \n Nabidh Status: {nabidh_status}")

    def action_t02(self):
        self.ensure_one()
        nabidh_status = self.client_id.original_document_notification('T02', appointment=self)
        self.message_post(body="T02/T08 sent to NABIDH. \n Nabidh Status:" + str(nabidh_status))

    def action_t08(self):
        self.ensure_one()
        nabidh_status = self.client_id.edit_notification('T08',appointment=self)
        self.message_post(body=f"T08(Edit Notification) sent to NABIDH. \n Nabidh Status: {nabidh_status}")

    def action_O01(self):
        self.ensure_one()
        nabidh_status = self.client_id.oder_message_O01('O01',appointment=self)
        self.message_post(body=f"O01(Order Message) sent to NABIDH. \n Nabidh Status: {nabidh_status}")
...
class AppointmentsCancel(models.TransientModel):
    ...
    def action_cancel_send(self):
        ...
        if 'appointment_id' in self._context:
            appo_obj = self.env['appointment.appointment'].browse(int(self._context.get('appointment_id')))
