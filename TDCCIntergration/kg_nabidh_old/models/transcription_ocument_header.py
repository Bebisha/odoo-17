...
class ZTranscriptionDocumentHeader(models.Model):
    ...
    document_completion_status_id= fields.Many2one('document.completion.status','Document Completion Status' ,default= lambda self: self.env['document.completion.status'].search([('code', '=', 'CP')], limit=1))
    document_availability_status_id= fields.Many2one('document.available.status','Document Availability Status' ,default= lambda self: self.env['document.available.status'].search([('code', '=', 'AV')], limit=1))
