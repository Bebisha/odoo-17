...
class AppointmentsAppointment(models.Model):
    ...
    def action_t02(self):
        ...
        self.message_post(body=f"T02(Original Document Notification) sent to NABIDH. Status: {nabidh_status}")

    def action_t08(self):
        self.ensure_one()
        nabidh_status = self.client_id.edit_notification('T08',appointment=self)
        self.message_post(body=f"T08(Edit Notification) sent to NABIDH. Status: {nabidh_status}")

    def action_O01(self):
        self.ensure_one()
        nabidh_status = self.client_id.oder_message_O01('O01',appointment=self)
        self.message_post(body=f"O01(Order Message) sent to NABIDH. Status: {nabidh_status}")
