...
class HelpdeskSLAStatus(models.Model):
    ...
    sla_stage_id = fields.Many2one('helpdesk.ticket.stage', related='sla_id.stage_id',
    ...
    exceeded_hours = fields.Float("Exceeded Working Hours", compute='_compute_exceeded_hours', compute_sudo=True,
                                  store=True,
    ...
    def _compute_deadline(self):
        for status in self:
            ...
            if time_days > 0:
                ...
                deadline = deadline and deadline.replace(hour=create_dt.hour, minute=create_dt.minute,
                                                         second=create_dt.second, microsecond=create_dt.microsecond)
    ...
    def _get_freezed_hours(self, working_calendar):
        ...
        tracking_lines = self.ticket_id.message_ids.tracking_value_ids.filtered(
            lambda tv: tv.field_id == field_stage).sorted(key="create_date")
