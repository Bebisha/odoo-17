...
class AgedPartnerBalanceCustomHandler(models.AbstractModel):
    ...
    def _report_custom_engine_aged_receivable(self, expressions, options, date_scope, current_groupby, next_groupby,
        ...
        return self._aged_partner_report_custom_engine_common(options, 'asset_receivable', current_groupby,
                                                              next_groupby, offset=offset, limit=limit)

    def _report_custom_engine_aged_payable(self, expressions, options, date_scope, current_groupby, next_groupby,
                                           offset=0, limit=None, warnings=None):
        return self._aged_partner_report_custom_engine_common(options, 'liability_payable', current_groupby,
                                                              next_groupby, offset=offset, limit=limit)
    ...
    def _aged_partner_report_custom_engine_common(self, options, internal_type, current_groupby, next_groupby, offset=0,
        ...
        def build_result_dict(report, query_res_lines):
            ...
            if current_groupby == 'id':
                ...
                expected_date = len(query_res['expected_date']) == 1 and query_res['expected_date'][0] or len(
                    query_res['due_date']) == 1 and query_res['due_date'][0]
                rslt.update({
    ...
    def change_expected_date(self, options, params=None):
        ...
        if aml.move_id.move_type == 'out_invoice':
            ...
            move_msg = _('Expected payment date for journal item %r has been changed from %s to %s on journal entry %r',
                         aml.name, old_date, new_date, aml.move_id.name)
...
class AgedPayableCustomHandler(models.AbstractModel):
    ...
    def _custom_unfold_all_batch_data_generator(self, report, options, lines_to_expand_by_function):
        ...
        if self.env.ref('account_reports.aged_payable_line').groupby.replace(' ', '') == 'partner_id,id':
            return self._common_custom_unfold_all_batch_data_generator('liability_payable', report, options,
                                                                       lines_to_expand_by_function)
        return {}
...
class AgedCombinedCustomHandler(models.AbstractModel):
    _name = 'account.aged.combined.report.handler'
    _inherit = ['account.aged.partner.balance.report.handler', 'account.aged.partner.balance.report.handler']
    _description = 'account.aged.combined.report'
