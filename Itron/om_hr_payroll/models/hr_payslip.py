...
class HrPayslip(models.Model):
    ...
    def get_amount_to_text(self,net_salary):
        amount_in_words = num2words(net_salary, to='currency', lang='en')
        amount_in_words = amount_in_words.replace("euro", "rupees").replace("cents", "paise")
        amount_in_words = amount_in_words.capitalize()
        return amount_in_words

    def get_total_con_in_words(self):
        for record in self:
            # Convert the total_con value to words in INR and capitalize it
            amount_in_words = num2words(record.contract_id.total_con, to='currency', lang='en')
            amount_in_words = amount_in_words.replace("euro", "rupees").replace("cents", "paise")
            record.total_con_in_words = amount_in_words.capitalize()

    total_con_in_words = fields.Char(string="Total Con In Words", compute="get_total_con_in_words")
    ...
    @api.onchange('employee_id', 'worked_days_line_ids')
    def _onchange_employee_id(self):
        ...
        if self.employee_id and self.employee_id.contract_id:
            ...
            basic_salary = self.employee_id.contract_id.wage
            print(basic_salary,'gfghfghfgh')
            daily_rate = basic_salary / 30
            print(daily_rate,"dddddddddddsw")

            for line in self.worked_days_line_ids.filtered(lambda x: x.code == 'GLOBAL'):
                unpaid_amount = line.number_of_days * -daily_rate
                self.input_line_ids = [(0, 0, {
                    'name': self.env.ref('om_hr_payroll.other_input_type_unpaid').id,
                    'code': self.env.ref('om_hr_payroll.other_input_type_unpaid').code,
...
class HrPayslipInput(models.Model):
    ...
    # name = fields.Char(string='Description', required=True)
    name = fields.Many2one('hr.payslip.input.type',string="Description")
    payslip_id = fields.Many2one('hr.payslip', string='Pay Slip', required=True, ondelete='cascade', index=True)
    sequence = fields.Integer(required=True, index=True, default=10)
    code = fields.Char(required=True, help="The code that can be used in the salary rules")
...
class HrPayslipInputTypes(models.Model):
