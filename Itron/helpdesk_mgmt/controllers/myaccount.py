...
class CustomerPortalHelpdesk(CustomerPortal):
    def _prepare_home_portal_values(self, counters):
        ...
        if "ticket_count" in counters:
            ...
            ticket_count = (helpdesk_model.search_count([]) if helpdesk_model.check_access_rights("read", raise_exception=False) else 0)
    ...
    def portal_my_tickets(self, page=1, date_begin=None, date_end=None, sortby=None, filterby=None, **kw):
        ...
        HelpdeskTicket = request.env["helpdesk.ticket"].sudo().search([])
        ...
        for stage in request.env["helpdesk.ticket.stage"].sudo().search([]):
        ...
        # pager
        pager = portal_pager(url="/my/tickets",url_args={},total=ticket_count,page=page,step=self._items_per_page,)
        ...
        restrict_ticket_enabled = any(amc_records.mapped('restrict_ticket'))
        current_engagement_period = None
        planned_hrs_period = 0.0
        for amc in amc_records:
            for period in amc.engagement_period_ids:
                if period.period_start_date and period.period_end_date:
                    if period.period_start_date <= date.today() <= period.period_end_date:
                        current_engagement_period = f"{period.period_start_date.strftime('%d-%m-%Y')} - {period.period_end_date.strftime('%d-%m-%Y')}"
                        planned_hrs_period = period.planned_hours
                        break
            if current_engagement_period:
                break
        total_spent_hrs = sum(amc_records.mapped('spent_hrs'))
        total_support_spent_hrs = sum(amc_records.mapped('support_spent_hrs'))
        ...
        total_support_hrs = sum(amc_records.mapped('support_hrs'))
        amc_success_pack = request.env['pack.projects'].sudo().search([
            ('partner_id', '=', request.env.user.partner_id.id),
            ('start_date', '<=', date.today()),
            ('end_date', '>=', date.today()),('stages','=','in_progress')
        ])
        total_enhancement_hrs = sum(amc_records.mapped('enhancement_hrs'))
        if restrict_ticket_enabled:
            if total_planned_hrs != 0 and total_spent_hrs >= total_planned_hrs:
                can_create_ticket = False
            else:
                can_create_ticket = True
            for amc_success_packs in amc_success_pack:
                if amc_success_packs and amc_success_packs.remaining_hours > 0:
                    can_create_ticket = True
        else:
            can_create_ticket = True
        values.update(
            {
                "date": date_begin,
                "tickets": tickets,
                "planned_hours": total_planned_hrs,
                "spent_hours": total_spent_hrs,
                "can_create_ticket": can_create_ticket,
                "period": current_engagement_period,
                "planned_hrs_period": planned_hrs_period,
                "page_name": "ticket",
                "support_spent_hrs": total_support_spent_hrs,
                "support_hrs": total_support_hrs,
                "enhancement_hrs": total_enhancement_hrs,
                "enhancement_spent_hrs": total_enhancement_spent_hrs,
                "pager": pager,
    ...
    def _ticket_get_page_view_values(self, ticket, **kwargs):
        closed_stages = request.env["helpdesk.ticket.stage"].sudo().search([("closed", "=", True)])
        values = {
            "page_name": "ticket",
            "ticket": ticket,
