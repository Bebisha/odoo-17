...
def get_route_and_shift(user_id):
    shift_id = user_id._get_active_shift()
    if not shift_id:
        raise UserError(_("No shift found for today."))
    planner_id = user_id._get_active_route_plan()
    if not planner_id:
        raise UserError(_("No planned route found for this sales driver. Please contact system administrator"))
    return shift_id, planner_id.route_id
