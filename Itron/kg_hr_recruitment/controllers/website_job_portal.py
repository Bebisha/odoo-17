...
class WebsiteHrRecruitmentJobPortal(http.Controller):
    _jobs_per_page = 12

    def sitemap_jobs(env, rule, qs):
        if not qs or qs.lower() in '/job_portal':
            yield {'loc': '/job_portal'}

    @http.route([
        '/job_portal',
        '/job_portal/page/<int:page>',
    ], type='http', auth="public", website=True, sitemap=sitemap_jobs)
    def jobs_portal(self, country_id=None, department_id=None, office_id=None, contract_type_id=None,
                    is_remote=False, is_other_department=False, is_untyped=None, page=1, search=None, **kwargs):
        """ Function for a custom job portal. """
        env = request.env(context=dict(request.env.context, show_address=True, no_tag_br=True))
        Country = env['res.country'].sudo()
        Jobs = env['hr.job'].sudo()
        Department = env['hr.department'].sudo()
        job_id = env['hr.job'].sudo().search([('is_published', '=', True)])
        ...
        # Render page
        return request.render("kg_hr_recruitment.index_job_portal", {
            'job_id': job_id,
            'jobs': jobs,
            'countries': countries,
    ...
    @http.route('''/job_portal/job_detail/<model("hr.job"):job>''', type='http', auth="public", website=True,
                sitemap=True)
    def jobs_detail(self, job, **kwargs):
        """ Function for a custom job details. """
        redirect_url = f"/job_portal/{slug(job)}"
        return request.redirect(redirect_url, code=301)

    @http.route('''/job_portal/<model("hr.job"):job>''', type='http', auth="public", website=True, sitemap=True)
    def job(self, job, **kwargs):
        """ Function for a custom job details. """
        return request.render("kg_hr_recruitment.job_detail", {
            'job': job,
            'main_object': job,
        })

    @http.route('''/job_portal/job_apply/<model("hr.job"):job>''', type='http', auth="public", website=True, sitemap=True)
    def jobs_apply(self, job, **kwargs):
        """ Function for a custom job apply. """
        error = {}
        default = {}
        if 'website_hr_recruitment_error' in request.session:
            error = request.session.pop('website_hr_recruitment_error')
            default = request.session.pop('website_hr_recruitment_default')
        return request.render("kg_hr_recruitment.job_apply", {
            'job': job,
            'error': error,
            'default': default,
        })
