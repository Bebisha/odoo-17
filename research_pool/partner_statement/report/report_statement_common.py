...
class ReportStatementCommon(models.AbstractModel):
    ...
    def _show_buckets_sql_q2(self, date_end, minus_30, minus_60, minus_90, minus_120):
        print(date_end, 'lllllllllllllll')
        return str(
            self._cr.mogrify(
                """
    ...
    def _get_report_values(self, docids, data=None):
        ...
        for key, entries in lines.items():
            for entry in entries:
                ...
                if move_id:
                    move_record = self.env['account.move'].search([('name', '=', move_id)])
                    if move_record.line_ids.sale_line_ids.order_id:
                        entry['against'] = move_record.line_ids.sale_line_ids.order_id.client_order_ref
                    else:
                        entry['against'] = False

                        print(move_record.line_ids.sale_line_ids.order_id,'move_record.line_ids.sale_line_ids.order_id')
                    if move_record.picking_id:
                        entry['picking_id'] = move_record.picking_id.name
                    else:
                        entry['picking_id'] = False

        # Print the updated lines
        print('Updated lines:', lines)
        ...
        for partner_id in partner_ids:
            ...
            for line in lines[partner_id]:
                # print(line,'kkkkkkkkkkkkkkkkkkk')
                ...
                if line["open_amount"] < 0:
                    line["credit"] = line["open_amount"]
                    line["debit"] = 0.00
                    credit_total = credit_total + line["open_amount"]
                if line["open_amount"] > 0:
                    line["credit"] = 0.00
                    line["debit"] = line["open_amount"]
                    debit_total = debit_total + line["open_amount"]

                print('line["date_maturity"]', line["date_maturity"],
                      'line["balance"]', line["balance"],
                      'line["credit"]', line["credit"],
                      'line["debit"]', line["debit"],
                      'line["open_amount"]', line["open_amount"], )

                line_currency["lines"].append(line)
                # line_currency["credit_total"].append(credit_total)
                # line_currency["debit_total"].append(debit_total
        ...
        print(partner_ids, "PARTNERRRR")
        salesperson_name = ''
        if salesperson_wise:
            partner_ids = partner_ids.filtered(lambda p: p.user_id.id == int(salesperson_id))
            salesperson_name = self.env["res.users"].browse(int(salesperson_id)).name
        pdc_data = []
        total = 0
        for i in partner_ids:
            pdc = self.env['pdc.wizard'].search([('partner_id', '=', i.id),('state','not in',['cancel','bounced'])])

            if pdc:
                for k in pdc:
                    total += k.payment_amount

                    pdc_data.append({'name': k.name,
                                     'payment_type': dict(k._fields['payment_type']._description_selection(self.env))[
                                         k.payment_type],
                                     'partner_id': k.partner_id.name,
                                     'payment_amount': k.payment_amount,
                                     'payment_date': k.payment_date,
                                     'ref': k.reference,
                                     'state': dict(k._fields['state']._description_selection(self.env))[
                                         k.state]})
                    print(pdc_data, 'pppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppp')

        return {
            "doc_ids": partner_ids,
            "doc_model": "res.partner",
            "docs": partner_ids,
            "salesperson_wise": salesperson_wise,
