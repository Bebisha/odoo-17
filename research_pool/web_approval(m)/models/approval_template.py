...
class ApprovalTemplateLine(models.Model):
    ...
    transaction_wise_user = fields.Boolean('Approval User Transaction Wise', default=False)
    ...
    def _notify_users(self):
        ...
        for rec in self:
            if not rec.transaction_wise_user:
                ...
                if model == 'purchase.order':
                    record_id.activity_schedule('web_approval.mail_activity_approval', note=note,
                                            user_id=record_id.project_rel_id and record_id.project_rel_id.user_id.id,
                                            date_deadline=today + relativedelta(days=rec.escalation_days))
                elif model == 'account.move':
                    usr = self.env['res.users']
                    for line in record_id.invoice_line_ids:
                        usr = line.purchase_order_id.project_rel_id and line.purchase_order_id.project_rel_id.user_id
                    if len(usr) == 1:
                        record_id.activity_schedule('web_approval.mail_activity_approval', note=note,
                                                        user_id=usr.id,
                                                        date_deadline=today + relativedelta(days=rec.escalation_days))
    ...
    def _check_amount_limit(self):
        ...
        if self.approval_id.approval_type == 'amount' and self.amount_limit != 0.0 and field_id:
    ...
    def _generate_upcoming_approval_json(self):
        ...
        for approval in self:
            if not approval._check_amount_limit():
                continue
            user_vals = []
            if not approval.transaction_wise_user:
                ...
                if model == 'purchase.order':
                    users = record_id.project_rel_id and record_id.project_rel_id.user_id
                    for user in users:
                        user_vals.append({
                            'id': user.id,
                            'name': user.name,
                ...
                if model == 'account.move':
                    ...
                    for line in record_id.invoice_line_ids:
                        users = line.purchase_order_id.project_rel_id and line.purchase_order_id.project_rel_id.user_id
                    for user in users:
                        user_vals.append({
                            'id': user.id,
                            'name': user.name,
