...
class PDC_wizard(models.Model):
    ...
    is_critical = fields.Boolean(
        string="Critical Date",
        compute="_compute_is_critical",
        store=False
    )

    @api.depends('cheque_clearance_date')
    ...
    def create(self, vals):
        if vals.get('payment_type') == 'receive_money':
            ...
            if journal_id:
                ...
                journal_prefix = f"P{journal_code}"
                sequence = self.env['ir.sequence'].next_by_code('pdc.customer.payment')
                vals['name'] = f"{journal_prefix}/{sequence}"
            ...
            if journal_id:
                journal = self.env['account.journal'].browse(journal_id)
                journal_code = journal.code
                journal_prefix = f"R{journal_code}"
