...
class EmployeeGratuity(models.Model):
    ...
    state = fields.Selection([
        ...
        ('approve', 'Approved'),
        ('cancel', 'Cancelled')],
        default='draft', tracking=True, help="Record state")
    ...
    employee_gratuity_duration = fields.Many2one('gratuity.'
                                                 'configuration',
                                                 readonly=True,
                                                 string='Configuration Line',
                                                 help="Choose valid "
                                                      "gratuity configuration")
    employee_gratuity_configuration = fields.Many2one('hr.'
                                                      'gratuity.accounting.'
                                                      'configuration',
                                                      readonly=True,
                                                      string='Gratuity '
    ...
    unused_leave_days = fields.Float(
        string="Unused Leave Days",
        compute="_compute_unused_leave_days",
        store=True
    )

    @api.depends('employee_id')
    def _compute_unused_leave_days(self):
        for rec in self:
            if not rec.employee_id:
                rec.unused_leave_days = 0.0
                continue

            # 1️⃣ Approved Leave Allocations
            ...
            # 3️⃣ Unused Leave
            rec.unused_leave_days = max(allocated_days - taken_days, 0.0)

    leave_salary_amount = fields.Float(
        string="Leave Salary",
        compute="_compute_leave_salary",
        store=True
    )

    @api.depends('employee_basic_salary', 'unused_leave_days')
    def _compute_leave_salary(self):
        for rec in self:
            daily = rec.employee_basic_salary / 30 if rec.employee_basic_salary else 0.0
            precision = rec.company_id.currency_id.decimal_places or 2
            rec.leave_salary_amount = float_round(
                daily * rec.unused_leave_days,
    ...
    def _onchange_employee_id(self):
        ...
        if self.employee_id.id:
            ...
            # configuration
            self.employee_gratuity_configuration = (
                hr_accounting_configuration_id.id)
            conf_ids = (hr_accounting_configuration_id.
                        gratuity_configuration_table.mapped('id'))
            ...
            if self.total_working_years < 1 and self.employee_id.id:
                raise UserError(_('Selected Employee is not eligible for '
                                  'Gratuity Settlement'))
            self.hr_gratuity_journal = (hr_accounting_configuration_id.
                                        gratuity_journal.id)
            self.hr_gratuity_credit_account = (hr_accounting_configuration_id.
                                               gratuity_credit_account.id)
            self.hr_gratuity_debit_account = (hr_accounting_configuration_id.
                                              gratuity_debit_account.id)
            ...
            if self.employee_gratuity_duration and self.wage_type == 'hourly':
                if self.employee_gratuity_duration.employee_working_days != 0:
                    ...
                    working_days_salary = (daily_wage *
                                           self.employee_gratuity_duration.
                    ...
                    raise UserError(_("Employee working days is not "
                                      ...
                                      "configuration..!"))
            elif (self.employee_gratuity_duration and
                  self.wage_type == 'monthly'):
                if (self.employee_gratuity_duration.
                        employee_daily_wage_days != 0):
                    daily_wage = (self.employee_basic_salary / self.
                                  employee_gratuity_duration.
...
class EmployeeContractWage(models.Model):
    """Inherited the Contract model for adding new fields"""
    ...
    company_country_id = fields.Many2one('res.country',
                                         ...
                                         readonly=True, help="Country "
                                                             ...
                                                             "company")
    wage_type = fields.Selection([('monthly', 'Monthly Fixed Wage'),
                                  ('hourly', 'Hourly Wage')], help="Choose "
                                                                   "wage type")
    hourly_wage = fields.Monetary('Hourly Wage', digits=(16, 2),
