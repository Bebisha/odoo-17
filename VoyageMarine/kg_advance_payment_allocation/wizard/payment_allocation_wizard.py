...
class PaymentAllocation(models.TransientModel):
    ...
    remaining_amount_new = fields.Float('Remaining Amount', compute="remaining_amount")
    ...
    @api.depends('invoice_allocation_ids', 'payment_allocation_ids')
    def remaining_amount(self):
        payment_amount = 0
        allocation_amount = 0
        for rec in self.payment_allocation_ids:
            for val in self.invoice_allocation_ids:
                allocation_amount += val.inv_allocate_amount
            payment_amount += rec.amount
            self.remaining_amount_new = payment_amount - allocation_amount
    ...
    def validate_payment(self):
        ...
        for rec in self.payment_allocation_ids:
            for val in self.invoice_allocation_ids:
                allocation_amount += val.inv_allocate_amount
            payment_amount += rec.amount
        if payment_amount < allocation_amount:
            raise ValidationError(_('Allocate amount exceeds Advance amount'))
...
class DebitLines(models.TransientModel):
    ...
    alrdy_allocated = fields.Float('Already Allocated Amount', store=1)
