...
class OutstandingStatement(models.AbstractModel):
    ...
    def _display_lines_sql_q1(self, partners, date_end, account_type):
        ...
        return str(
            ...
            SELECT l.id, m.name AS move_id, l.partner_id, l.date, l.name,
                            l.blocked, l.currency_id, l.company_id, m.amount_total, m.amount_residual_signed, m.amount_residual,
                            
            CASE WHEN m.vessel_id IS NOT NULL
                THEN vc.name
                ELSE ''
            END as vessel_id,
            ...
            LEFT JOIN vessel_code vc ON (vc.id = m.vessel_id)
            ...
            GROUP BY l.id, l.partner_id, m.name, l.date, l.date_maturity, l.name, m.vessel_id, vc.name, m.amount_total, m.amount_residual_signed, m.amount_residual,
                CASE WHEN l.ref IS NOT NULL
                    THEN l.ref
                    ELSE m.ref
                END,
        ...
        return str(
            self._cr.mogrify(
                ...
                SELECT Q1.partner_id, Q1.currency_id, Q1.move_id,
                    ...
                    Q1.name, Q1.ref, Q1.blocked, Q1.company_id, Q1.vessel_id, Q1.amount_total, Q1.amount_residual_signed, Q1.amount_residual,
    ...
    def _display_lines_sql_q3(self, company_id):
        return str(
            self._cr.mogrify(
                """
            SELECT Q2.partner_id, Q2.move_id, Q2.date, Q2.date_maturity,Q2.vessel_id, Q2.amount_total, Q2.amount_residual_signed, Q2.amount_residual,
    ...
    def _get_account_display_lines(
        ...
        self.env.cr.execute(
            ...
            WITH Q1 as (%s),
                 Q2 AS (%s),
                 Q3 AS (%s)
            SELECT partner_id, currency_id, move_id, date, date_maturity, debit,vessel_id,amount_total, amount_residual_signed, amount_residual,
