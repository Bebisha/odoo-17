...
class ReportStatementCommon(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
            ...
            for entry in entries:
                ...
                if move_id:
                    ...
                    if move_record.line_ids.sale_line_ids.order_id:
                        ...
                        # entry['vessel'] = move_record.line_ids.sale_line_ids.order_id.vessel_id.name
                    else:
                        entry['against'] = False
                        # entry['vessel'] = False
        ...
        pdc_data = []
        total = 0
        for i in data["partner_ids"]:
            pdc = self.env['pdc.wizard'].search(
                [('partner_id', '=', i), ('state', 'in', ['draft', 'registered', 'deposited']),
                 ('company_id', '=', data['company_id']), ('due_date', '<=', data['date_end'])])
