...
class OutstandingStatementXslx(models.AbstractModel):
    _name = "report.partner_statement.report_outstanding_statement_xlsx"
    ...
    def _write_currency_lines(self, row_pos, sheet, partner, currency, data):
        ...
        debit_total = data.get("debit_total", False)
        credit_total = data.get("credit_total", False)
        row_pos += 2
        ...
        sheet.merge_range(
            row_pos, 0, row_pos, 5, statement_header, FORMATS["format_right_bold"]
        )
        row_pos += 1
        sheet.write(
            row_pos, 1, _("LPONO / DESCRIPTION"), FORMATS["format_theader_yellow_center"]
        )
        ...
        sheet.write(
            row_pos, 2, _("AGAINST"), FORMATS["format_theader_yellow_center"]
        )
        sheet.write(row_pos, 3, _("DEBIT AMOUNT"), FORMATS["format_theader_yellow_center"])
        sheet.write(
            row_pos, 4, _("CREDIT AMOUNT"), FORMATS["format_theader_yellow_center"]
        )
        sheet.write(row_pos, 5, _("BALANCE AMOUNT"), FORMATS["format_theader_yellow_center"])
        for line in currency_data.get("lines"):
            ...
            sheet.write(
                row_pos, 3, line.get("debit", ""), FORMATS["current_money_format"]
            )
            sheet.write(
                row_pos, 4, line.get("credit", ""), FORMATS["current_money_format"]
            )
            sheet.write(
                row_pos, 5, line.get("balance", ""), FORMATS["current_money_format"]
            )
        row_pos += 1
        # sheet.write(
        ...
        # )
        sheet.merge_range(
            row_pos, 0, row_pos, 2, _("TOTAL OUTSTANDING"), FORMATS["format_theader_yellow_center"]
        )
        sheet.write(
            row_pos, 3, debit_total, FORMATS["format_current_money_format_yellow_right"]
        )
        sheet.write(
            row_pos, 4, credit_total, FORMATS["format_current_money_format_yellow_right"]
        )
        sheet.write(
            row_pos, 5, currency_data.get("amount_due"), FORMATS["format_current_money_format_yellow_right"]
        )
        return row_pos
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        for partner in partners:
            ...
            for currency_id in currencies:
                ...
                FORMATS["format_current_money_format_yellow_right"] = workbook.add_format(
                    dict(num_format=money_string, align="right", bold=True))
