...
class hr_employee(models.Model):
    ...
    user_type = fields.Selection([('report_user', 'Report User'), ('attendance_user', 'Attendance User')],
                                 'Mobile User Type')
    ...
    work_permit_name = fields.Char(string="Work Permit Name")
    ...
    def compute_leave_eligible_employee_master(self):
        ...
        for data in self:
            ...
            if date_from:
                ...
                leave_request_date = str(
                    datetime.strptime(leave_request_date, '%Y-%m-%d %H:%M:%S') + timedelta(hours=4))

            # calculating allowed leave for particular employee at the date of requesting
            worked_days_in_the_current_year = 0
            if date_from:
                worked_days_in_the_current_year = (datetime.strptime(year_end_date,
                                                                     '%Y-%m-%d %H:%M:%S') - datetime.strptime(
                    leave_request_date, '%Y-%m-%d %H:%M:%S')).days + 1
            ...
            leave_remaining = 0
            if date_from:
                allocated_ids = self.env['hr.leave.allocation'].search(
                    [('employee_id', '=', employee_id), ('holiday_status_id', '=', 1), ('state', '=', 'validate'),
                     ('date_from', '=', False)])
                if allocated_ids:
            ...
            if date_from:
                leave_ids_for_current_date = self.env['hr.leave'].search(
                    [('employee_id', '=', employee_id), ('holiday_status_id', '=', 1),
                     ('state', 'in', ('validate', 'od_resumption_to_approve', 'od_approved')),
                     ('date_from', '<', leave_request_date)])
...
class hr_employee_dependents_line(models.Model):
    ...
    benefits_ids = fields.Many2many('hr.employee.benefits', 'hr_dependants_benefits_rel', 'employee_id', 'benefits_id',
                                    string='Benefits')
...
class HrAttendance(models.Model):
                ...
                if self.employee_id.resource_calendar_id.global_leave_ids:
                    for glob in self.employee_id.resource_calendar_id.global_leave_ids:
                        checkin = datetime.strptime(
                            (self1.check_in + timedelta(hours=4, minutes=0, seconds=0)).strftime("%Y-%m-%d %H:%M:%S"),
                            '%Y-%m-%d %H:%M:%S')
                        if checkin >= glob.date_from and checkin <= glob.date_to:
                            return True
                        else:
                            sch = self.env['employee.schedule'].search([('emp_id', '=', self1.employee_id.id), (
                                'name', '=',
                                str(self1.check_in.strftime("%b")) + '-' + str(self1.check_in.strftime("%Y")))])
                            if sch:
                                todays_work = 0
                                att = self.env['hr.attendance'].search([('employee_id', '=', self1.employee_id.id)],
                                                                       order='check_in asc')
                                if att:
                                    for a in att:
                                        if (a.check_in + timedelta(hours=4, minutes=0, seconds=0)).strftime(
                                                "%Y-%m-%d") == str(self1.check_in.strftime("%Y-%m-%d")):
    ...
    def _get_diff(self):
        ...
        for self1 in self:
            ...
            if self1.check_in:
                sch = self.env['employee.schedule'].search([('emp_id', '=', self1.employee_id.id), (
                    'name', '=', str(self1.check_in.strftime("%b")) + '-' + str(self1.check_in.strftime("%Y")))])
                if sch:
                    work_hrs = sch.working_hrs
    ...
    def _compute_worked_hours(self):
        for attendance in self:
            ...
            if self.employee_id.resource_calendar_id.global_leave_ids:
                for glob in self.employee_id.resource_calendar_id.global_leave_ids:
                    checkin = datetime.strptime(
                        (attendance.check_in + timedelta(hours=4, minutes=0, seconds=0)).strftime("%Y-%m-%d %H:%M:%S"),
                        '%Y-%m-%d %H:%M:%S')
                    if checkin >= glob.date_from and checkin <= glob.date_to:
                        if attendance.check_out and attendance.check_in:
                            delta = attendance.check_out - attendance.check_in
    ...
    total_overtime = fields.Float('Total Overtime', compute='_count_total_overtime', store=True)
    ...
    def action_recalculate(self):
        for attendance in self:
            if attendance.check_out and attendance.check_in:
                ...
                attendance.worked_hours = False
            sch = self.env['employee.schedule'].search([('emp_id', '=', attendance.employee_id.id), (
                'name', '=', str(attendance.check_in.strftime("%b")) + '-' + str(attendance.check_in.strftime("%Y")))])
            if sch:
                work_hrs = sch.working_hrs
    ...
    def update_check_out(self):
        ...
        for lines in attendances:
            sch = self.env['employee.schedule'].search([('emp_id', '=', lines.employee_id.id), (
                'name', '=', str(lines.check_in.strftime("%b")) + '-' + str(lines.check_in.strftime("%Y")))])
            if sch:
                work_hrs = sch.working_hrs
            if lines.check_in:
...
class OvertimeData(models.Model):
    ...
    def _count_total_overtime(self):
        ...
        for self1 in self:
            if self1.date_work:
                sch = self.env['employee.schedule'].search([('emp_id', '=', self1.emp_id.id), (
                    'name', '=', str(self1.date_work.strftime("%b")) + '-' + str(self1.date_work.strftime("%Y")))])
                if sch:
                    work_hrs = sch.working_hrs
    ...
    def action_approve(self):
        ...
        for att in self:
            ...
            if attd:
                for a in attd:
                    if (a.check_in + timedelta(hours=4, minutes=0, seconds=0)).strftime("%Y-%m-%d") == str(
                            self.date_work):
                        a.state = 'approved'
...
class ResConfigSettings(models.TransientModel):
    ...
    schedular_time = fields.Integer(string='Schedular Time Interval', config_parameter='hr_custom.schedular_time',
                                    default=0.0)
