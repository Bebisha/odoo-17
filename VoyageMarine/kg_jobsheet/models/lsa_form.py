...
class KGLSAForm(models.Model):
    _name = 'lsa.form'
    _description = 'LSA Form'
    _inherit = ['mail.thread', 'mail.activity.mixin']

    name = fields.Char(string="LSA Number",copy=False, index=True, readonly=False,
    ...
    def _compute_form_counts_repair(self):
        for order in self:
            order.repair_form_count = self.env['repair.order'].search_count([('job_ref_lsa', '=', self.id)])
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            vals['name'] = self.env['ir.sequence'].next_by_code(
                'lsa.seq') or _('New')
        res = super(KGLSAForm, self).create(vals)
        return res

    @api.onchange('product_category_id')
    def _onchange_product_category_id_lsa(self):
    ...
    def create_lsa(self):
        if self.qty_lsa <= 0.0:
            raise ValidationError("The quality LSA cannot be zero or negative.")
        ...
        if existing_qty >= self.qty_lsa:
            raise ValidationError(
                f"The existing quantity of {existing_qty} exceeds or equals the requested quantity of {self.qty_lsa}.")

        remaining_qty = int(self.qty_lsa - existing_qty)
        if remaining_qty <= 0:
            raise ValidationError("No remaining quantity available for LSA.")

        # if existing_qty == 0:
        #     product_qty = self.qty_lsa
        # else:
        #     product_qty = self.qty_lsa - existing_qty
        # if product_qty <= 0:
        #     raise ValidationError("No remaining quantity available for LSA.")
    ...
    def action_lsa(self):
        return {
            ...
            'view_mode': 'tree,form',
            'res_model': 'repair.order',
            'domain': [('job_ref_lsa', '=', self.id)],
            'context': {'create': False}
        }

    def create_lsa_certificate(self):
        return self.env.ref("kg_jobsheet.action_lsa_report").report_action(self)