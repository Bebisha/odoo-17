...
class CrmEstimationItems(models.Model):
    _name = 'crm.estimation.item'
    _inherit = ['mail.thread', 'mail.activity.mixin']

    item_id = fields.Many2one('crm.estimation')
    ...
    main_product_id = fields.Many2one("crm.estimation.item", string="Main Product Reference")
    is_sub_item = fields.Boolean(default=False, string="Subitem")
    is_confirm = fields.Boolean(default=False, string="IS confirm", compute="compute_is_confirm", copy=False)
    sub_item_ids = fields.Many2many("crm.estimation.item", relation='crm_sub_item', column1='col1', column2='col2',
                                    string="Subitems", compute='compute_subitems', copy=True)
    ...
    currency_id = fields.Many2one("res.currency", related="item_id.currency_id", string="Currency")
    company_id = fields.Many2one("res.company", related="item_id.company_id", string="Company")
    ...
    def _update_main_unit_price_from_subitems(self):
        for rec in self:
            ...
            subitems = self.env['crm.estimation.item'].search([
    ...
    def write(self, vals):
        res = super(CrmEstimationItems, self).write(vals)
        for rec in self:
            # If THIS is a subitem → update parent
            if rec.main_product_id:
                rec.main_product_id._update_main_unit_price_from_subitems()

            # If quantity changed on main line → recalc
    ...
    def create(self, vals_list):
        records = super(CrmEstimationItems, self).create(vals_list)
        for rec in records:
            if rec.main_product_id:
                rec._recompute_parent_margin(rec.main_product_id)
        return records
    ...
    @api.onchange('item_id.item_ids', 'code')
    def _get_line_numbers(self):
        for line in self:
            line_num = 1
            for line_rec in line.item_id.item_ids:
    ...
    def compute_total_subitems(self):
        for rec in self:
            subitems_id = self.env['crm.estimation.item'].search([('main_product_id', '=', rec.id)])
            rec.total_subitems = sum(subitems_id.mapped('total')) + rec.total if subitems_id else rec.total

    def compute_is_confirm(self):
        for rec in self:
            if rec.main_product_id.item_id.status == 'confirm':
                rec.is_confirm = True
            else:
    ...
    def compute_subitems(self):
        for rec in self:
            subitems_id = self.env['crm.estimation.item'].search([('main_product_id', '=', rec.id)])
            if subitems_id:
                rec.sub_item_ids = [(6, 0, subitems_id.ids)]
                rec.subitems_total = sum(subitems_id.mapped('total'))
            else:
    ...
    def kg_add_subitems(self):
        ...
        if not self.item_id.is_qtn and self.item_id.so_ids:
    ...
    #                 rec.product_id.standard_price, rec.currency_id, rec.company_id, rec.item_id.estimate_date
    #             )
    ...
    def unlink(self):
        ...
        res = super(CrmEstimationItems, self).unlink()
