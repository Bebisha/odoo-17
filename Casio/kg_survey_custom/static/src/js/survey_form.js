    ...
    */
    _submitForm: async function (options) {
        ...
        if (this.options.isStartScreen) {
            route = "/survey/begin/";
            if(this.$('#exampleInputEmail1').val() == '')
            {
                this.$('.warning').show();
                this.$('.warning').text('Email Id Required');
                return false;
            }
            else
            {
             ...
             if(this.$('#InputMobile').val() == '')
            {
                this.$('.warning').show();
                this.$('.warning').text('Missing Mobile Number');
                return false;
            }
            else
            {
...
//        customization
        const submitPromise = this.rpc(
            `${route}${this.$('#exampleInputEmail1').val()}/${this.$('#InputMobile').val()}/${this.options.surveyToken}/${this.options.answerToken}`,
            params
        );
//        original
//        const submitPromise = this.rpc(
