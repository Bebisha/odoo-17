...
class GMusicClubController(http.Controller):

    @http.route('/get_city', type='json', auth='public', methods=['POST'], csrf=False)
    def get_city(self, **post):
        ...
        cities = http.request.env['res.partner'].sudo().get_city(country)
        return list(cities)

    @http.route(['/join_gmusic_club', '/join_gmusic_club/<int:agent_id>'], type='http', auth='public', website=True)
    def join_gmusic_club(self, agent_id=None):
        agent_name = False
        ...
        if agent_name not in ['EMI Events', 'EMI Online']:
            default_region = 'middle'
            set_readonly = True
        return request.render('kg_gshock_page.join_gmusic_club_template', {
            'agent_id': agent_id,
    ...
    @http.route(['/scan_gmusic_club', '/scan_gmusic_club/<int:agent_id>'], type='http', auth='public', website=True)
    def scan_gshock_club(self, agent_id=None):
        agent_name = ''
        if agent_id:
            agent_order_id = http.request.env["agent.order"].sudo().search([('id', '=', agent_id)], limit=1)
            agent_name = agent_order_id.agent_name
        return request.render('kg_gshock_page.scan_gmusic_club_template',
                              {'agent_id': agent_id, 'agent_name': agent_name,
                               'city_id': 'empty',
                               'country_id': 'empty'
                               })

    @http.route('/g-music/privacy-policy', type='http', auth='public', website=True)
    def gshock_privacy(self, **kw):
        return request.render('kg_gshock_page.gm_privacy_policy_page', {})

    @http.route("/gm/submit_form", type="http", auth="public", website=True, csrf=False)
    def submit_form(self, **kw):
        ...
        vals = {
            ...
            "email": kw.get("email"),
            "mobile": str(kw.get("phonecode_id"))+str(kw.get("phone")),
            ...
            "skill_level":kw.get("skill_level"),
            "kg_join_club":"music"
        }
        if partner_exist:
        ...
        return request.render('kg_gshock_page.gm_thank_you_page', {'agent_id': kw.get('agent_id')})
