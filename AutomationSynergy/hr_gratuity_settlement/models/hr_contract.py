...
class HrContract(models.Model):
    ...
    def create(self, vals_list):
        ...
        details in a model
        """
        if 'trial_date_end' in vals_list and vals_list['trial_date_end'] and vals_list['state'] == 'probation':
    ...
    def write(self, vals):
        ...
        for rec in self:
            if rec.state == 'probation':
                if vals.get('state') == 'open' and not rec.is_approve:
            ...
            if not training_dtl:
                if rec.trial_date_end and rec.state == 'probation':
