...
class AirTicket(models.Model):
    ...
    # @api.constrains('leave_id')
    # def leave_constrains(self):
    #     if self.leave_id:
    #         rejoin = self.env['air.ticket'].search([('leave_id', '=', self.leave_id.id)])
    #         if len(rejoin) > 1:
    #             raise UserError(_("This leave is already linked"))
    ...
    def action_confirm(self):
        ...
        # if self.employee_id.employee_type_new == 'office':
        #     if any(vals.departure_date.year == self.departure_date.year for vals in tkt):
        #         raise ValidationError(('Ticket already raised for the year %s') % self.departure_date.year)
        # elif self.employee_id.employee_type_new == 'site':
        #     if any((vals.departure_date.year == self.departure_date.year) or (
        #             vals.departure_date.year == self.departure_date.year - 1) for vals in tkt):
        #         raise ValidationError(
        #             ('Ticket already raised for %s or %s') % (self.departure_date.year, self.departure_date.year - 1))
        emp_yr = 0
