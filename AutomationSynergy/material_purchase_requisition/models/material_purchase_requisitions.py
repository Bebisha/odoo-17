...
class MaterialPurchaseRequisition(models.Model):
    ...
    delivery_picking_count = fields.Integer('Internal Picking Count', )
    ...
    # @api.depends('requisition_line_ids.is_fully_available')
    # def _compute_all_fully_available(self):
    #     for requisition in self:
    #         requisition.all_fully_available = all(line.is_fully_available for line in requisition.requisition_line_ids)
    #
    ...
    def kg_request(self):
        dm_users = []
        unit_prices = self.requisition_line_ids.mapped('price_unit')
        if any(price == 0 for price in unit_prices):
            raise ValidationError("One or more products have zero unit price. Please check and correct them.")

        dm_approve_users = self.env['ir.config_parameter'].sudo().get_param(
            'kg_hr.department_manager_ids', False)
    ...
    def _get_picking_count(self):
        for picking in self:
            picking.delivery_picking_count = False
...
class RequisitionLine(models.Model):
    ...
    po_quantity = fields.Float(string="PO Qty", help="Quantity to purchase", default=0)
    ...
    def get_purchase_qty(self):
        for rec in self:
            rec.purchase_quantity = False
            purchase = self.env['purchase.order'].search([]).filtered(
                lambda x: rec.requisition_id.id == x.requisition_po_id.id)
            ...
            total_ordered_qty = sum(order_lines.mapped('product_qty'))
