...
class HrWps(models.TransientModel):
    ...
    def print_xlsx(self):
        ...
        company = self.env['res.company']._company_default_get('hr.wps')
        ...
        slips = self.env['hr.wps'].get_data(self.start_date, self.end_date)
        if not slips:
    ...
    def get_data(self, start, end):
        ...
        slips_ids = self.env['hr.payslip'].search(['&',
                                                   ...
                                                   ('date_to', '<=', end)]). \
            sorted('id')
        for slip in slips_ids:
            slip_data = []
            total_allowance = 0
            total_deduction = 0
            slip_data.append(slip.employee_id.labour_card_number)
            slip_data.append(slip.employee_id.salary_card_number)
            slip_data.append(slip.employee_id.agent_id.routing_code)
            basic_salary = slip.line_ids.filtered(lambda l: l.category_id.code == 'BASIC').total
            slip_data.append(basic_salary)
            allowance = slip.line_ids.filtered(lambda l: l.category_id.code == 'ALW')
            total_allowance += sum(allowance.mapped('total'))
            slip_data.append(total_allowance)
            deduction = slip.line_ids.filtered(lambda l: l.category_id.code == 'DED')
            total_deduction += sum(deduction.mapped('total'))
            slip_data.append(total_deduction)
            net_salary = slip.line_ids.filtered(lambda l: l.category_id.code == 'NET').total
            slip_data.append(net_salary)
            working_days = sum(slip.worked_days_line_ids.mapped('number_of_days'))
            slip_data.append(working_days)
            data.append(tuple(slip_data))
        return data
    ...
    def get_xlsx_report(self, lines, response):
        ...
        sheet.write(count, 2, 'Bank Routing Code', format1)
        ...
        sheet.write(count, 7, 'Basic', format1)
        sheet.write(count, 8, 'Allowance', format1)
        sheet.write(count, 9, 'Deduction', format1)
        sheet.write(count, 10, 'Amount', format1)
        sheet.write(count, 11, 'Currency', format1)
        # sheet.write(count, 12, 'Leaves', format1)
        for count in enumerate(dat):
            ...
            sheet.write(count[0]+1, 1, count[1][0], format0)
            sheet.write(count[0]+1, 2, count[1][2], format0)
            sheet.write(count[0]+1, 3, count[1][1], format0)
            sheet.write(count[0]+1, 4, lines['start_date'], format0)
            sheet.write(count[0]+1, 5, lines['end_date'], format0)
            sheet.write(count[0]+1, 6, str(int(days)).zfill(2), format0)
            v_sum += float(count[1][6])
            sheet.write(count[0] + 1, 7, count[1][3], format0)
            sum_basic += count[1][3]
            sheet.write(count[0] + 1, 8, count[1][4], format0)
            sum_allowance += count[1][4]
            sheet.write(count[0] + 1, 9, count[1][5], format0)
            sum_deduction += count[1][5]
            sheet.write(count[0]+1, 10, count[1][6], format0)
            sheet.write(count[0]+1, 11, 'AED', format0)
            # sheet.write(count[0]+1, 12, leaves, format0)
        count = count[0] + 3
        ...
        sheet.write(count, 4, time[0] + str(':') + time[1], format0)
        month_year = str(lines['end_date']).split('-')
        month_year = str(month_year[1]) + str('-') + str(month_year[0])
        sheet.write(count, 5, month_year, format0)
        sheet.write(count, 6, '', format0)
        sheet.write(count, 7, sum_basic, format0)
        sheet.write(count, 8, sum_allowance, format0)
        sheet.write(count, 9, sum_deduction, format0)
        sheet.write(count, 10, v_sum, format0)
        sheet.write(count, 11, 'AED', format0)
        workbook.close()
        output.seek(0)
