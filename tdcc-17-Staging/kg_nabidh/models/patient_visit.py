...
class PatientVisit(models.Model):
    ...
    def _compute_full_name(self):
        for record in self:
            print("Patient Visit ID:", record.patient_visit_id)  # Debug: Check the patient visit record
            if record.patient_visit_id:
                # Check if the patient visit record has a physician_id
                physician = record.patient_visit_id.physician_id  # Ensure this is fetching the related physician
                if physician:
                    print("Physician ID:", physician)  # Debug: Check the physician record
                    ...
                    # Full name in format: DR. Lastname Firstname
                    record.full_name = f"Dr. {record.last_name} {record.middle_name}{record.first_name}".strip()
                else:
