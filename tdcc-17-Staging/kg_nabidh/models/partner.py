...
class Partner(models.Model):
    ...
    def get_pid_t02(self, message_type):
        PID = {}
        ...
        for pat_iden in self.patient_identifer_ids:
            for parties in self.associated_parties_ids:
                pid = {
                    ...
                    "name": {
                        "firstName": str(pat_iden.full_name) if pat_iden.full_name else '',
                        'GivenName': str(pat_iden.first_name) if pat_iden.first_name else '',
                        "lastName": str(pat_iden.last_name) if pat_iden.last_name else ''
                    },
                    "address": {
                        "streetAddress": str(parties.street) if parties.street else '',
        ...
        for app in self.app_app_ids:
            ...
            for patient in app.patient_visit_ids:
                for patient_vs in app.hospital_service:
                    pv1 = {
                        "class": patient.patient_class.name if patient.patient_class.name else '',
                        "assignedPatientLocation": patient.assigned_patient_location if patient.assigned_patient_location else '',
                    }
                    PV1.update(pv1)
    ...
    def get_pv1_t02(self, message_type):
        PV1 = {}
    ...
    def get_obx_t02(self, message_type):
        OBX = []
        for app in self.app_app_ids:
            for observation in app.observations_ids:
                base64_string = ''
                ...
                obx = {
                    ...
                    "observationId": observation.set_id_obx,
                    "valueType": "NM",
    ...
    def get_rxo_o01(self, message_type):
        RXO = {}
        for app in self.app_app_ids:
            for pharmacy in app.pharmacy_order_ids:
                rxo = {
    ...
    def get_nte_t02(self, message_type):
        NTE = []
    ...
    def get_txa(self, message_type):
        TXA = {}

        for app in self.app_app_ids:
            for txa in app.transcription_document_ids:
                txa = {
    ...
    def get_values_O01(self, message_type):
        demo_list = []

        MSH = self.get_msh(message_type)
        EVN = self.get_evn(message_type)
        PID = self.get_pid_vo4(message_type)
        PV1 = self.get_pv1_ao3(message_type)
        ORC = self.get_orc_o01(message_type)
        RXO = self.get_rxo_o01(message_type)
    ...
    def oder_message_O01(self):
        if not self.is_vip_sent:
            if self.is_vip:
                self.is_vip_sent = True

            values = self.get_values_O01('O01')
            ...
            if o01_response:
                ...
                self.message_post(body="O01 request send to NABIDH")
            else:
    ...
    def get_values_t02(self, message_type):
        demo_list = []
        MSH = self.get_msh(message_type)
        PID = self.get_pid_t02(message_type)
        PV1 = self.get_pv1_t02(message_type)
        TXA = self.get_txa(message_type)
        OBX = self.get_obx_t02(message_type)
        NTE = self.get_nte_t02(message_type)
        # ZSC = self.get_zsc(message_type)

        demo_list.append({"MSH": MSH})
        demo_list.append({"PID": PID})
        demo_list.append({"PV1": PV1})
        demo_list.append({"TXA": TXA})
        demo_list.append({"OBX": OBX})
        demo_list.append({"NTE": NTE})
    ...
    def edit_notification(self):
        if not self.is_vip_sent:
            ...
            values = self.get_values_t02('T08')
            ...
            if t08_response:
                ...
                message.action_send(t08_result['Details'], t08_result['HL7Message'], self.id)
                self.message_post(body="T08 request sent to NABIDH")
            else:
