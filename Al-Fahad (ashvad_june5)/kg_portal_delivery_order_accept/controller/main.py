...
class CustomerPortal(portal.CustomerPortal):
    ...
    def vendor_lists_views(self, page=1, sortby='name', search="", search_in="all", **kw):
        ...
        if request.env.user.has_group('stock.group_stock_manager'):
            ...
            search_domain = search_list.get(search_in, {}).get('domain', [
                ...
                ('warehouse_id', '=', request.env.user.property_warehouse_id.id),
                ('state', 'in', ['assigned', 'done'])
            ])
    ...
    def create_invoice(self, picking_id=None):
        try:
            delivery_order = request.env['stock.picking'].sudo().browse(picking_id)

            # Check if the delivery order exists
            if not delivery_order.exists():
                return "Delivery order not found!"

            # Check if the delivery order is in the "done" state
            if delivery_order.state != 'done':
                return "Delivery order is not in the 'done' state. Cannot create invoice."
                ...
                for line in delivery_order.move_ids_without_package:
                    if line.product_id:
                        invoice_line_vals.append((0, 0, {
                            ...
                            'sale_line_ids': [(6, 0, [sale_line.id]) for sale_line in
                                              delivery_order.sale_id.order_line],
                        }))
            else:
                for line in delivery_order.move_ids_without_package:
                    if line.product_id:
            ...
            template = request.env.ref('kg_portal_delivery_order_accept.invoice_email_template_inherit')
            if template:
                base_url = request.env['ir.config_parameter'].sudo().get_param('web.base.url')
                base_url += '/web#id=%d&view_type=form&model=%s' % (invoice.id, invoice._name)

                template.sudo().send_mail(invoice.id, force_send=True, email_values=email_values)
                print("Email sent successfully.")
            else:
                print("Failed to find email template.")

            delivery_order.sudo().boolean = True
