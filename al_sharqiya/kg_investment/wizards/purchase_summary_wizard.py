...
class PurchaseSummaryWizard(models.TransientModel):
    ...
    def purchase_summary_report_button(self):
        """ To print pdf report """
        if self.report_type == 'yearly':
            ...
            purchase_orders = self.env['kg.purchase.order'].search(
                [('state', '=', 'posted'), ('investment_type_id', '=', self.investment_type.id),('company_id','=',self.company_id.id)])
            investments = purchase_orders.mapped('investment_id').sorted(
                key=lambda x: x.name)
            year_wise = {}
            ...
            for investment in investments:
                ...
                for year in range(start_year, end_year + 1):
                    ...
                    purchases = purchase_orders.filtered(
                        lambda p: p.date.year == year and p.investment_id == investment)
                    no_shares = sum(p.no_of_shares for p in purchases)
                    cost = 0.0
                    ...
                    if market:
                        for market_line in market.market_rate_line_id:
                            if market_line.investment_id.name == investment.name:
                                ...
                                cost = market_rate
                                # cost = sum(p.no_of_shares * market_rate for p in purchases)
                                break
                    else:
                        cost = 0.0
                    ...
                    year_totals[year]['share_value'] += round(share_value, 6)
        ...
        return self.env.ref('kg_investment.action_report_purchase_summary').with_context(
    ...
    def get_monthly_records(self):
        ...
        investments = purchase_orders.mapped('investment_id').sorted(key=lambda x: x.name)
        month_wise = {}
        months_list = []
        ...
        while current_date <= to_date:
            ...
            for investment in investments:
                ...
                month_purchases = purchase_orders.filtered(
    ...
    def purchase_summary_report_button_xlsx(self):
        if self.report_type == 'yearly':
            start_year = int(self.from_year)
            end_year = start_year + self.range
            if start_year > date.today().year:
            ...
            for investment in investments:
                ...
                for year in range(start_year, end_year + 1):
                    date_start = date(year, 1, 1)
                    date_end = date(year, 12, 31)
                    purchases = purchase_orders.filtered(lambda p: p.date.year == year and p.investment_id == investment)
                    no_shares = sum(p.no_of_shares for p in purchases)
                    cost = 0.0
                    market = self.env['market.rate.updation'].search([
                        ('investment_type_id', '=', investment.investment_type.id),
        ...
        worksheet = workbook.add_sheet('Purchase Summary')
        ...
        worksheet.write_merge(0, 1, 4, 8, 'Yearly Purchase/Buy Analysis', bold_style_large)
        if self.report_type == 'yearly':
            ...
            for month_index, month in enumerate(month_wise['months_list']):
                ...
                worksheet.write(grand_total_row, col + 2, month_wise['month_totals'][month]['share_value'], bold_style_small)
        output = BytesIO()
        workbook.save(output)
        output.seek(0)
        attachment = self.env['ir.attachment'].create({
            'name': 'Purchase_Summary_Report.xls',
            'type': 'binary',
